 ...
 */
class Table extends commonGrpc.ServiceObject {
  ...
  createReadStream(options) {
    ...
    if (options.keys || options.ranges.length) {
      ...
      if (options.ranges.length) {
        reqOpts.rows.rowRanges = options.ranges.map(range => Filter.createRange(range.start, range.end, 'Key'));
      }
    }
    ...
    return pumpify.obj([
      ...
      through.obj(function(data, enc, next) {
        ...
        const rows = Row.formatChunks_(data.chunks, {
  ...
  getFamilies(callback) {
    ...
    this.getMetadata((err, resp) => {
      ...
      const families = Object.keys(resp.columnFamilies).map(familyId => {
