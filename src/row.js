 ...
 */
class Row extends commonGrpc.ServiceObject {
  ...
  create(entry, callback) {
    ...
    this.parent.mutate(entry, (err, apiResponse) => {
      if (err) {
  ...
  filter(filter, onMatch, onNoMatch, callback) {
    ...
    this.request(grpcOpts, reqOpts, (err, apiResponse) => {
      if (err) {
        callback(err, null, apiResponse);
        return;
      }
  ...
  get(columns, options, callback) {
    ...
    if (columns.length) {
      const filters = columns.map(Mutation.parseColumnName).map(column => {
        const filters = [{family: column.family}];
    ...
    this.parent.getRows(reqOpts, (err, rows, apiResponse) => {
      if (err) {
        callback(err, null, apiResponse);
        return;
      }
  ...
  increment(column, value, callback) {
    ...
    this.createRules(reqOpts, (err, resp) => {
      if (err) {
        callback(err, null, resp);
        return;
      }
  ...
  save(key, value, callback) {
    ...
    if (is.string(key)) {
      const column = Mutation.parseColumnName(key);
 ...
 * // }
 */
Row.formatChunks_ = (chunks, options) => {
  const rows = [];
  let familyName;
  let qualifierName;
 ...
 * // }
 */
Row.formatFamilies_ = (families, options) => {
  const data = {};
