 ...
 */
class Instance extends commonGrpc.ServiceObject {
  ...
  createCluster(name, options, callback) {
    ...
    this.request(protoOpts, reqOpts, (err, resp) => {
      ...
      const bigtable = self.parent;

      const cluster = self.cluster(name);
  ...
  createTable(name, options, callback) {
    ...
    this.request(protoOpts, reqOpts, (err, resp) => {
      ...
      const table = self.table(resp.name);
      table.metadata = resp;
  ...
  getTables(query, callback) {
    ...
    this.request(protoOpts, reqOpts, (err, resp) => {
      ...
      const tables = resp.tables.map(metadata => {
        const name = metadata.name.split('/').pop();
